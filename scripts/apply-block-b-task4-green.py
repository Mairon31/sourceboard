...
old_touch = '''    if (at - current.lastUsedAt >= SESSION_TOUCH_INTERVAL_MS) {
      await dependencies.store.touchSession(current.id, at);
    }'''
service_text = Path(service).read_text()
if service_text.count(old_touch) != 2:
    raise SystemExit(f"{service}: expected two touch blocks, found {service_text.count(old_touch)}")
service_text = service_text.replace(
    old_touch, "    await refreshSessionContext(current, context.request, at);", 1
)
service_text = service_text.replace(old_touch, "    await refreshSessionContext(current, request, at);", 1)
Path(service).write_text(service_text)
replace_once(
    service,
    '''    const createdAt = now();
