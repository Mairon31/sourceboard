-- Expanded built-in Store catalog. Idempotent by deterministic ids.
...
VALUES ('source-hunters', 'source-hunters', 'Source Hunters', 'DISABLED', unixepoch('now') * 1000);

-- Emote packs become visible only after an admin publishes a populated pack.
UPDATE store_items
SET is_active = 0, updated_at = unixepoch('now') * 1000
WHERE id = 'store-emotes'
  AND NOT EXISTS (SELECT 1 FROM emote_catalog WHERE pack_id = 'source-hunters' AND status = 'ACTIVE');

INSERT INTO system_metadata (key, value, created_at, updated_at)
VALUES ('store.catalog.version', '2026-09-06-store-v1', unixepoch('now') * 1000, unixepoch('now') * 1000)
