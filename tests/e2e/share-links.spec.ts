...
test.describe("stable share links", () => {
  ...
  test("browser follows the short link and preserves a supported locale", async ({ page, request }) => {
    const shortUrl = await createNavigationPostShare(request);
  ...
  test("missing short IDs return an unavailable response without target metadata", async ({ request }) => {
