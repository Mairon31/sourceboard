...
test("Discovery switches List, Gallery and Detailed Grid over the same results", async ({ page }) => {
  seedCategoryBadgeFixture();
  ...
  expect(Math.abs((galleryBounds?.width ?? 0) - (galleryBounds?.height ?? 0))).toBeLessThanOrEqual(2);
  ...
  const animeCard = page.locator('[data-search-view="grid"] [data-search-post-id="e2e-category-anime-post"]');
