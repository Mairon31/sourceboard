...
describe("post detail viewer Like state", () => {
  it("projects the authenticated viewer's persisted Like into SSR post detail", () => {
    expect(route).toContain('import { readViewerLikedPostIds } from "../data/viewer-post-likes";');
    expect(route).toContain("await readViewerLikedPostIds(runtime.db, userId, [post.id])");
