...
describe("deterministic CMS seed", () => {
  it("keeps the committed migration content-identical to the deterministic generator", () => {
    expect(normalizeLineEndings(generatedSeed())).toBe(normalizeLineEndings(committedSeed));
  });

  it("seeds every static article without dropping its English content", () => {
    for (const article of DOCS_ARTICLES) {
      expect(committedSeed).toContain(
        `'seed:${article.kind === "policy" ? "legal" : "docs"}:${article.slug}'`,
      );
