...
describe("notifications route", () => {
  it("maps the supported filters explicitly", () => {
    ...
    expect(filterNotificationCards(cards, "UNREAD").map((item) => item.type)).toEqual([
      "post.liked",
    ]);
    expect(filterNotificationCards(cards, "ACTIVITY").map((item) => item.type)).toEqual([
      "post.liked",
    ]);
    expect(filterNotificationCards(cards, "SOCIAL").map((item) => item.type)).toEqual([
      "friend.request",
    ]);
    expect(filterNotificationCards(cards, "SYSTEM").map((item) => item.type)).toEqual([
      "moderation.action",
    ]);
  });
  ...
  it("renders shared cards and all five initial filters", () => {
    const source = readFileSync(
      new URL("../../app/routes/notifications.tsx", import.meta.url),
      "utf8",
    );
