...
| XSS                         | Covered with residual CSP work         | React escapes rendered text, rich text is parsed into an allowlisted DTO and external links are constrained. CSP blocks objects/frames/scripts outside the allowlist, but the theme bootstrap currently requires `'unsafe-inline'`; migrate it to a nonce before launch. |
...
1. CSP still permits inline script/style for the existing theme bootstrap and
   SSR output. This is a documented launch blocker for a nonce migration, not a
   claim of complete XSS containment.
